...
float textureGood(sampler2D sam, vec2 uv) {
    ...
    float uv_px_base_y_min = mod(uv_px_base.y, res.y);
    float uv_px_base_y_max = mod(uv_px_base.y+1.0, res.y);
...
void main() {
  ...
  // get the floatwise velocity in toroidal coordinates
  ...
  vec3 vel_crt = u_minorRadius*vel_sph.x*e_theta_hat + vel_sph.y*(u_majorRadius + u_minorRadius * cos(theta))*e_phi_hat;
  ...
  float extrapolated_old_theta = atan(old_pos_crt.z, length(old_pos_crt.xy) - u_majorRadius);
...
void main() {
  ...
  float u_majorRadius = 0.5;
  float u_minorRadius = 0.25;
  float theta = v_texCoord.x * 2.0 * PI - PI;
  vec2 pGradient = vec2(
      (1.0/(u_majorRadius + u_minorRadius * cos(theta)))*(p21 - p01)/res.x,
      (1.0/u_minorRadius)*(p12 - p10)/res.y
  );
  //vec2 pGradient = vec2(
      //(p21 - p01)/res.x,
      //(p12 - p10)/res.y
  ...
  vec2 vel = texture(u_vel_tex, v_texCoord).xy - pGradient/15.0;
  value = vec4(vel, 0, 0);
}
`
...
class IncompressibleTorusFluidDemo extends React.Component<IncompressibleTorusFluidDemoProps, IncompressibleTorusFluidDemoState> {
  ...
  animationLoop = () => {
    ...
    if (this.needsVelocityReset) {
      ...
      this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, this.scalarFramebuffers[this.scalarIndex]);
      const destScalar = new Float32Array(this.props.xsize * this.props.ysize * 4);
      this.gl.readPixels(0, 0, this.props.xsize, this.props.ysize, this.gl.RGBA, this.gl.FLOAT, destScalar);
      console.log(destScalar);
